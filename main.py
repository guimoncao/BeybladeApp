...
# ==========================================
# 2. MOTOR DA NUVEM BLINDADO (Com RLock Anti-Travamento)
# ==========================================
FIREBASE_URL = "https://beybladeapp-c303a-default-rtdb.firebaseio.com/beyblade_data.json"

app_data = {"bladers": [], "tournament": None, "history": [], "training_history": [], "users": {}, "last_updated": 0}
is_syncing = False 
db_lock = threading.RLock() # 👑 RLock impede travamentos caso uma função chame a outra
...
# 👑 FUNÇÕES DE BANCO DE DADOS DESCOMPACTADAS (Evita bugs)
def get_bladers():
    with db_lock: return json.loads(json.dumps(app_data.get("bladers", [])))
def save_bladers(bl_list): 
    with db_lock: app_data["bladers"] = bl_list
    save_db(app_data)
...
def save_tournament(t_data): 
    with db_lock: app_data["tournament"] = t_data
    save_db(app_data)
...
def set_active_match(m_data):
    with db_lock: app_data["active_match"] = m_data
    save_db(app_data)

def get_history():
    with db_lock: return json.loads(json.dumps(app_data.get("history", [])))
def add_to_history(t_data):
...
def add_to_training_history(train_data):
...
def save_users(u_dict):
    with db_lock: app_data["users"] = u_dict
    save_db(app_data)
...
def PrimaryBtn(text, on_click, width=None, height=48, icon=None, data=None, color=C_PRIMARY, expand=False):
    items = [ft.Icon(icon, color=C_TEXT_PRI, size=20)] if icon else []
...
def SecondaryBtn(text, on_click, width=None, height=48, icon=None, data=None, expand=False):
    items = [ft.Icon(icon, color=C_TEXT_SEC, size=20)] if icon else []
...
def main(page: ft.Page):
    ...
    page.appbar = ft.AppBar(title=ft.Text("Beyblade X", size=16, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI), bgcolor=C_BG, actions=[ft.Container(content=sync_ring, padding=ft.padding.only(right=10)), ft.IconButton(ft.Icons.LOGOUT, icon_color=C_ERROR, on_click=do_logout, tooltip="Sair")])
    ...
    def build_auth_view(is_login=True):
        ...
        def do_auth(e):
            ...
            if not u or not p:
                page.snack_bar = ft.SnackBar(ft.Text("Preencha tudo!"), bgcolor=C_ERROR); page.snack_bar.open = True; page.update(); return
            safe_cloud_sync()
            db_users = get_users()
            if is_login:
                ...
                if u in HARDCODED_USERS or u in db_users:
                    page.snack_bar = ft.SnackBar(ft.Text("Usuário já existe!"), bgcolor=C_ERROR); page.snack_bar.open = True; page.update(); return
                db_users[u] = {"password": p, "email": e_input.value.strip(), "role": "basic"}
                save_users(db_users)
        ...
        return ft.Column([
            ...
            ft.Text("Motor oficial de torneios.", size=14, color=C_TEXT_SEC),
    ...
    def build_profile_view():
        return ft.Container(padding=24, content=ft.Column([ft.Container(height=50), ft.Icon(ft.Icons.ACCOUNT_CIRCLE, size=80, color=C_TEXT_SEC), ft.Text(f"Olá, {get_username()}!", size=24, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI), ft.Container(height=10), Badge("Acesso Básico", C_PRIMARY), ft.Container(height=20), ft.Text("Sua conta permite usar a aba Combate para contador de partidas casuais.", color=C_TEXT_SEC, text_align="center"), ft.Text("Peça a um Administrador para elevar sua conta para Pro.", color=C_PRIMARY, text_align="center")], horizontal_alignment=ft.CrossAxisAlignment.CENTER))

    # --- TELA 1: TREINO ISOLADO (PRO) ---
    def build_training_view():
        if not is_pro(): return ft.Container(content=ft.Text("Acesso Restrito. Exclusivo PRO.", color=C_ERROR), padding=24)

        if training_state["sub_tab"] == "combat":
            m_data = training_state["match_data"]
            score_p1 = ft.Text(str(training_state["p1_score"]), size=64, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI)
            score_p2 = ft.Text(str(training_state["p2_score"]), size=64, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI)
            ...
            def process_win():
                ...
                def finish_match(e):
                    ...
                    # 👑 TRABALHO ASSÍNCRONO: Fecha a tela rápido e processa depois para evitar congelamento!
                    def async_save_training():
                        safe_cloud_sync()
                        record = {
                            "id": str(int(time.time()*1000)),
                            "DATA": datetime.now().strftime('%d/%m/%Y %H:%M'),
                            "NOME_TREINO": m_data["treino_nome"],
                            "COMBO_TESTADO": m_data["combo_testado"],
                            "COMBO_ADVERSARIO": m_data["combo_adversario"],
                            "LADO_ARENA": m_data["lado_arena"],
                            "MEU_XTREME": training_state["p1_finishes"]["xtreme"], "MEU_BURST": training_state["p1_finishes"]["burst"], "MEU_OVER": training_state["p1_finishes"]["over"], "MEU_SPIN": training_state["p1_finishes"]["spin"], "MEU_FLAG": training_state["p1_finishes"]["flag"],
                            "ADV_XTREME": training_state["p2_finishes"]["xtreme"], "ADV_BURST": training_state["p2_finishes"]["burst"], "ADV_OVER": training_state["p2_finishes"]["over"], "ADV_SPIN": training_state["p2_finishes"]["spin"], "ADV_FLAG": training_state["p2_finishes"]["flag"],
                            "PLACAR": f'{training_state["p1_score"]} x {training_state["p2_score"]}', 
                            "RESULTADO": "Vitória" if training_state["p1_score"] > training_state["p2_score"] else "Derrota"
                        }
                        add_to_training_history(record)
                        training_state["sub_tab"] = "setup"
                        refresh_current_tab()
            ...
            def add_points(player, pts, t_finish):
                if training_state["match_ended"]: return 
                if player == 1:
                    training_state["p1_score"] += pts
                    training_state["p1_finishes"][t_finish] += 1
                    score_p1.value = str(training_state["p1_score"])
                else:
                    training_state["p2_score"] += pts
                    training_state["p2_finishes"][t_finish] += 1
                    score_p2.value = str(training_state["p2_score"])
                page.update()
            ...
            def reset_match(e=None):
                ...
                training_state["p1_finishes"] = {"spin": 0, "over": 0, "burst": 0, "xtreme": 0, "flag": 0}
                training_state["p2_finishes"] = {"spin": 0, "over": 0, "burst": 0, "xtreme": 0, "flag": 0}
                training_state["match_ended"] = False
                score_p1.value, score_p2.value = "0", "0"
                page.update()
                
            def cancel_training(e):
                training_state["sub_tab"] = "setup"
                refresh_current_tab()

            def action_col(p):
                return ft.Column([
                    PrimaryBtn(f"XTREME (+{PTS_MAP['xtreme']})", lambda _: add_points(p, PTS_MAP['xtreme'], "xtreme"), width=145, height=44, color=C_PRIMARY),
                    SecondaryBtn(f"BURST (+{PTS_MAP['burst']})", lambda _: add_points(p, PTS_MAP['burst'], "burst"), width=145, height=44),
                    SecondaryBtn(f"OVER (+{PTS_MAP['over']})", lambda _: add_points(p, PTS_MAP['over'], "over"), width=145, height=44),
                    SecondaryBtn(f"SPIN (+{PTS_MAP['spin']})", lambda _: add_points(p, PTS_MAP['spin'], "spin"), width=145, height=44),
                    SecondaryBtn(f"FLAG (+{PTS_MAP['flag']})", lambda _: add_points(p, PTS_MAP['flag'], "flag"), width=145, height=44),
                ], spacing=12, alignment=ft.MainAxisAlignment.START, horizontal_alignment=ft.CrossAxisAlignment.CENTER)

            return ft.Container(padding=0, content=ft.Column([
                ft.Container(content=ft.Row([IconButton(ft.Icons.ARROW_BACK, cancel_training), ft.Text(f"TREINO: {m_data['treino_nome'].upper()}", color=C_TEXT_PRI, weight=ft.FontWeight.W_600, size=13, expand=True, text_align="center"), ft.Container(width=42)]), bgcolor=C_SUCCESS, padding=12, width=float("inf")),
            ...
            def start_training(e):
                ...
                training_state["match_data"] = {
                    "treino_nome": treino_nome_input.value.strip(),
                    "combo_testado": combo_test_input.value.strip(),
                    "combo_adversario": combo_adv_input.value.strip(),
                    "lado_arena": lado_dropdown.value
                }
                training_state["p1_score"], training_state["p2_score"] = 0, 0
                training_state["p1_finishes"] = {"spin": 0, "over": 0, "burst": 0, "xtreme": 0, "flag": 0}
                training_state["p2_finishes"] = {"spin": 0, "over": 0, "burst": 0, "xtreme": 0, "flag": 0}
                training_state["match_ended"] = False
                training_state["sub_tab"] = "combat"
                refresh_current_tab()
            ...
            def confirm_delete_training(t_id, t_name):
                def do_delete(e):
                    ...
                    with db_lock:
                        ...
                        app_data["training_history"] = [h for h in hist if h.get("id") != t_id]
                    save_db(app_data)
                    hide_dialog(dlg)
                    refresh_current_tab()
            ...
            def trigger_export(e):
                ...
                if not hist:
                    page.snack_bar = ft.SnackBar(ft.Text("Nenhum histórico para exportar."), bgcolor=C_ERROR); page.snack_bar.open = True; page.update(); return
                
                filename = f"Treino_Historico_{int(time.time())}.csv"
                ...
                if success_path:
                    dlg = ft.AlertDialog(title=ft.Text("Sucesso!"), content=ft.Text(f"Planilha exportada em:\n\n{success_path}", size=13), actions=[ft.TextButton("OK", on_click=lambda _: hide_dialog(dlg))])
                    show_dialog(dlg)
                else:
                    try:
                        ...
                        with open(filepath, mode='w', encoding='utf-8-sig', newline='') as f:
                            ...
                            writer.writerow(headers)
                            for row in hist: writer.writerow([row.get(h, "0") for h in headers])
                        dlg = ft.AlertDialog(title=ft.Text("Aviso"), content=ft.Text(f"Salvo na pasta do app:\n\n{filepath}", size=13), actions=[ft.TextButton("OK", on_click=lambda _: hide_dialog(dlg))])
                        show_dialog(dlg)
                    except Exception:
                        dlg = ft.AlertDialog(title=ft.Text("Erro"), content=ft.Text("Sem permissão para salvar no sistema."), actions=[ft.TextButton("OK", on_click=lambda _: hide_dialog(dlg))])
                        show_dialog(dlg)
            ...
            if not hist: hist_list.controls.append(ft.Text("Nenhum treino registrado ainda.", color=C_TEXT_SEC))

            return ft.Container(
                padding=24,
                content=ft.Column([
                    ft.Text("Laboratório de Combos (PRO)", size=24, weight=ft.FontWeight.BOLD, color=C_PRIMARY),
                    ft.Text("Configure os combos para iniciar a partida na arena.", size=13, color=C_TEXT_SEC),
                    ft.Container(height=10), treino_nome_input, combo_test_input, combo_adv_input, lado_dropdown,
                    PrimaryBtn("Ir para Arena de Treino", start_training, width=float("inf"), icon=ft.Icons.PLAY_ARROW),
                    ft.Divider(color=C_BORDER, height=20),
                    ft.Row([ft.Text("Histórico", size=18, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI, expand=True), SecondaryBtn("Exportar CSV", trigger_export, icon=ft.Icons.DOWNLOAD)]),
                    ft.Container(content=hist_list, expand=True)
                ])
            )

    # --- TELA 3: BLADERS (PRO/ADMIN) ---
    def build_home_view():
        ...
        def add_blader(e):
            ...
            if blader_input.value.strip():
                is_adding = True; safe_cloud_sync(); b_list = get_bladers()
                b_list.append({"id": str(int(time.time())), "name": blader_input.value.strip()})
                save_bladers(b_list); home_state["temp_name"] = ""; blader_input.value = ""; refresh_current_tab(); is_adding = False
        ...
        def confirm_remove_blader(b_id, b_name):
            def do_remove(e):
                ...
                hide_dialog(dlg_confirm); refresh_current_tab()
        ...
        def open_config_view(e):
                    ...
                    def make_btn(idx, delta):
                        def on_click(e):
                            if config_state["sizes"][idx] + delta >= 1:
                                config_state["sizes"][idx] += delta; update_dist_ui()
            ...
            def confirm_create(e):
                if sum(config_state["sizes"]) != total_b:
                    page.snack_bar = ft.SnackBar(ft.Text("⚠️ Ajuste as vagas!"), bgcolor=C_ERROR); page.snack_bar.open = True; page.update(); return
                safe_cloud_sync() 
                groups = []
    ...
    def build_tournament_view():
        ...
        # 👑 INTERFACE DE COMBATE DO TORNEIO (RODANDO AQUI DENTRO)
        if tourn_state["sub_tab"] == "combat":
            m_data = tourn_state["active_match"]
            state = tourn_state["match_state"]
            ...
            def process_win():
                ...
                def finish_match(e):
                    ...
                    def async_save_match():
                        ...
                        save_tournament(fresh_t) 
                        tourn_state["sub_tab"] = "partidas"
                        tourn_state["active_match"] = None
                        refresh_current_tab() 
                    
                    threading.Thread(target=async_save_match, daemon=True).start()

                dlg = ft.AlertDialog(modal=True, bgcolor=C_SURFACE, shape=ft.RoundedRectangleBorder(radius=16), title=ft.Text(f"🏆 Vitória de {winner_name}!", color=C_PRIMARY, weight=ft.FontWeight.BOLD), content=ft.Text("Partida concluída e computada na nuvem.", color=C_TEXT_SEC), actions=[PrimaryBtn("Confirmar e Voltar", finish_match, width=float("inf"))])
                show_dialog(dlg)
            ...
            return ft.Container(
                padding=0,
                content=ft.Column([
                    ft.Container(content=ft.Row([IconButton(ft.Icons.ARROW_BACK, cancel_match), ft.Text(header_txt, color=C_TEXT_PRI, weight=ft.FontWeight.W_600, size=13, expand=True, text_align="center"), ft.Container(width=42)]), bgcolor=header_col, padding=12, width=float("inf")),
                    ft.Container(padding=24, expand=True, content=ft.Column([
                        ft.Row([ft.Column([p1_display, score_p1, action_col(1)], expand=True, horizontal_alignment=ft.CrossAxisAlignment.CENTER), ft.Container(width=1, bgcolor=C_BORDER, height=300, margin=ft.margin.symmetric(horizontal=8)), ft.Column([p2_display, score_p2, action_col(2)], expand=True, horizontal_alignment=ft.CrossAxisAlignment.CENTER)], alignment=ft.MainAxisAlignment.CENTER, vertical_alignment=ft.CrossAxisAlignment.START),
                        ft.Container(expand=True), SecondaryBtn("Resetar Placar", reset_match, width=float("inf"), icon=ft.Icons.REFRESH)
                    ]))
                ])
            )
            
        else:
            # 👑 EXIBIÇÃO NORMAL DA TABELA DO TORNEIO
            ...
            def go_to_match(e):
                # 👑 MUDA O ESTADO INTERNO DO TORNEIO (NÃO VAI MAIS PRA ABA COMBATE)
                ...
                if res.status_code == 200:
                    ...
                    if nuvem:
                        ...
                        if needs_refresh and bottom_nav.destinations:
                            ...
                            if current_label not in ["Bladers", "Treino", "Combate", "Torneio"]: 
                                refresh_current_tab()
            except Exception:
                pass 
