...
# ==========================================
# 1. DESIGN SYSTEM E CONSTANTES DO JOGO
# ==========================================
C_BG = "#0B0B0F"          
C_SURFACE = "#15161A"     
...
# Cores e Constantes de Pontuação
C_XTREME = C_PRIMARY
C_BURST = "#A855F7"       
C_OVER = "#3B82F6"        
C_SPIN = "#22C55E"        
...
# ==========================================
# 2. BASE DE DADOS E MOTOR DA NUVEM
# ==========================================
...
def PrimaryBtn(text, on_click, width=None, height=48, icon=None, data=None, color=C_PRIMARY, expand=False):
    ...
    return ft.Container(
        ...
        bgcolor=color, padding=8, border_radius=12, alignment=ft.Alignment(0, 0), width=width, height=height, on_click=on_click, data=data, expand=expand
    )
...
def SecondaryBtn(text, on_click, width=None, height=48, icon=None, data=None, expand=False):
    ...
    return ft.Container(
        content=ft.Row(items, alignment=ft.MainAxisAlignment.CENTER, spacing=8) if icon else items[0],
        bgcolor=C_SURFACE_SEC, padding=8, border_radius=12, border=ft.border.all(1, C_BORDER), alignment=ft.Alignment(0, 0), width=width, height=height, on_click=on_click, data=data, expand=expand
    )

def IconButton(icon, on_click, color=C_TEXT_SEC):
    return ft.Container(
        content=ft.Icon(icon, color=color, size=22), padding=10, border_radius=10, bgcolor=C_SURFACE_SEC, border=ft.border.all(1, C_BORDER), on_click=on_click
    )

def Badge(text, color):
    return ft.Container(
        content=ft.Text(text, size=11, weight=ft.FontWeight.W_600, color=color), padding=6, bgcolor=f"{color}15", border_radius=6, border=ft.border.all(1, f"{color}40")
    )
...
def main(page: ft.Page):
    ...
    page.appbar = ft.AppBar(
        title=ft.Text("Beyblade X Counter", size=16, weight=ft.FontWeight.BOLD, color=C_TEXT_PRI),
        bgcolor=C_BG, actions=[ft.Container(content=sync_ring, padding=16)]
    )

    def _show_sync():
        sync_ring.visible = True; page.update()
    def _hide_sync():
        sync_ring.visible = False; page.update()
    ...
    # --- TELA 1: INÍCIO (CADASTRO E CONFIGURAÇÃO) ---
    def build_home_view():
        bladers = get_bladers()
        
        def save_temp_name(e):
            home_state["temp_name"] = e.control.value

        blader_input = ft.TextField(
            value=home_state["temp_name"], hint_text="Nome do Blader...", expand=True, bgcolor=C_SURFACE_SEC, border_color=C_BORDER, color=C_TEXT_PRI, text_size=14, border_radius=12, content_padding=16, cursor_color=C_PRIMARY, on_change=save_temp_name
        )
        ...
        def confirm_remove_blader(b_id, b_name):
            ...
            dlg_confirm = ft.AlertDialog(
                bgcolor=C_SURFACE, shape=ft.RoundedRectangleBorder(radius=16), title=ft.Text("Confirmar Exclusão", color=C_TEXT_PRI), content=ft.Text(f"Tem certeza que deseja remover '{b_name}' do banco de dados?", color=C_TEXT_SEC), actions=[SecondaryBtn("Cancelar", lambda _: hide_dialog(dlg_confirm)), PrimaryBtn("Remover", do_remove, color=C_ERROR)]
            )
            show_dialog(dlg_confirm)

        bladers_list_ui = ft.ListView(expand=True, spacing=12)
        for b in bladers:
            bladers_list_ui.controls.append(AppCard(ft.Row([ft.Text(b["name"], weight=ft.FontWeight.W_500, color=C_TEXT_PRI, size=15, expand=True), IconButton(ft.Icons.DELETE_OUTLINE, lambda e, bid=b["id"], bname=b["name"]: confirm_remove_blader(bid, bname), color=C_ERROR)]), padding=12))
        ...
        def open_config_view(e):
            ...
            def confirm_create(e):
                ...
                for i, size in enumerate(config_state["sizes"]):
                    ...
                    # 🎲 EMBARALHAMENTO DOS CONFRONTOS NO GRUPO (Evita 3 partidas seguidas da mesma pessoa)
    ...
    # --- TELA 2: PARTIDA RÁPIDA (MÁXIMA PERFORMANCE - ZERO DELAY) ---
    def build_quick_match_view():
        ...
        def process_win():
            ...
            dlg = ft.AlertDialog(
                ...
                content=ft.Text("A partida foi concluída e computada.", color=C_TEXT_SEC),
        ...
        def action_col(p):
            return ft.Column([
                PrimaryBtn(f"XTREME (+{PTS_MAP['xtreme']})", lambda _: add_points(p, PTS_MAP['xtreme'], "xtreme"), width=145, height=44),
        ...
        def advance_to_knockout(e):
            ...
            for g in fresh_tourn.get("groups", []):
                if not all(m.get("completed") for m in g.get("matches", [])):
                    page.snack_bar = ft.SnackBar(ft.Text(f"⚠️ Finalize todas as partidas do {g.get('name', 'Grupo')} antes de avançar!"), bgcolor=C_ERROR)
            ...
            for i in range(0, p2, 2):
                ...
                m = {
                    "id": f"r0-m{i//2}-{int(time.time())}", "blader1": p1, "blader2": p2_id, "completed": is_bye,
                }
            ...
            for r in range(1, rounds):
                ...
                for i in range(len(prev_matches) // 2):
                    ...
                    next_matches.append({"id": f"r{r}-m{i}-{int(time.time())}", "blader1": b1, "blader2": b2, "completed": False})
                knockout.append({"name": round_names.get(rounds - r, f"Rodada {r+1}"), "matches": next_matches})

            if rounds >= 2:
                knockout[-1]["matches"].append({
                    "id": f"r{rounds-1}-m1-{int(time.time())}", "name": "Disputa de 3º Lugar", "blader1": None, "blader2": None, "completed": False
                })
            ...
            page.snack_bar = ft.SnackBar(ft.Text("Dica: Em caso de empates técnicos na Fase de Grupos, o sistema sorteou aleatoriamente. Se precisar, ajuste manualmente as vitórias antes de avançar."), bgcolor=C_PRIMARY)
            page.snack_bar.open = True; page.update()
        ...
        for group in tourn.get("groups", []):
            ...
            for match in group.get("matches", []):
                ...
                if match.get("completed"):
                    ...
                    status_ui = ft.Row([ft.Text(f"{pts1} - {pts2}", color=C_PRIMARY, weight=ft.FontWeight.BOLD, size=16), IconButton(ft.Icons.INFO_OUTLINE, lambda e, md=match: open_match_details(md, tourn))])
                else:
            ...
            for r_idx, round_data in enumerate(tourn.get("knockout", [])):
                ...
                for match in round_data.get("matches", []):
                    ...
                    if match.get("completed"):
                        ...
                        if b1_id is None or b2_id is None: status_ui = ft.Text("Aguardando...", color=C_TEXT_SEC, size=12)
                        else: status_ui = PrimaryBtn("Jogar", go_to_match, height=36, width=80, data={"is_knockout": True, "round_idx": r_idx, "match_id": match.get("id"), "b1_id": b1_id, "b1_name": b1_name, "b2_id": b2_id, "b2_name": b2_name})
        ...
        return ft.Container(
            ...
            content=ft.Column([
                ft.Container(content=ft.Row([
                    ...
                    ft.Row([
                        IconButton(ft.Icons.REFRESH, manual_refresh, color=C_PRIMARY),
    ...
    def refresh_current_tab(): 
        if bottom_nav.selected_index != 0:
